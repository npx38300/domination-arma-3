//#define __DEBUG__
// by Xeno
#define THIS_FILE "x_m24.sqf"
#include "x_setup.sqf"

d_x_sm_pos = "d_sm_24" call d_fnc_smmapos; // index: 24,   Fuel station in camp near Kalochori
d_x_sm_type = "normal"; // "convoy"

if (!isDedicated && {!d_IS_HC_CLIENT}) then {
	d_cur_sm_txt = localize "STR_DOM_MISSIONSTRING_844";
	d_current_mission_resolved_text = localize "STR_DOM_MISSIONSTRING_758";
};

if (call d_fnc_checkSHC) then {
	private ["_vehicle", "_poss"];
	_poss = d_x_sm_pos select 0;
	_vehicle = createVehicle [d_fuel_station, _poss, [], 0, "NONE"];
	_vehicle setDir (markerDir "d_sm_24");
	_vehicle setPos _poss;
	_vehicle call d_fnc_addKilledEHSM;
	sleep 2.22;
	["aa", 1, "tracked_apc", 1, "tank", 1, _poss,1,350,true] spawn d_fnc_CreateArmor;
	sleep 2.123;
	["specops", 1, "basic", 2, _poss,250,true] spawn d_fnc_CreateInf;
	d_x_sm_vec_rem_ar pushBack _vehicle;
};
