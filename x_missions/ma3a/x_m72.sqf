...
if (!isDedicated && {!d_IS_HC_CLIENT}) then {
	d_cur_sm_txt = format [localize "STR_DOM_MISSIONSTRING_1562", "Molos"];
...
if (call d_fnc_checkSHC) then {
	...
	_vehicle setDir (markerDir "d_sm_72");
	_vehicle setPos _poss;
	sleep 2.123;
	["specops", 3, "basic", 1, _poss,50,true] spawn d_fnc_CreateInf;
	sleep 2.221;
	["aa", 2, "tracked_apc", 1, "tank", 0, d_x_sm_pos select 1, 1, 50, true] spawn d_fnc_CreateArmor;
