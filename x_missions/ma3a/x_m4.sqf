//#define __DEBUG__
// by Xeno
#define THIS_FILE "x_m4.sqf"
#include "x_setup.sqf"

d_x_sm_pos = "d_sm_4" call d_fnc_smmapos; // index: 4,   Cargo truck (chemical weapons) in Nifi
d_x_sm_type = "normal"; // "convoy"

if (!isDedicated && {!d_IS_HC_CLIENT}) then {
	d_cur_sm_txt = localize "STR_DOM_MISSIONSTRING_858";
	d_current_mission_resolved_text = localize "STR_DOM_MISSIONSTRING_787";
};

if (call d_fnc_checkSHC) then {
	private ["_vehicle", "_poss"];
	_poss = d_x_sm_pos select 0;
	_vehicle = createVehicle [d_sm_cargo, _poss, [], 0, "NONE"];
	_vehicle setDir (markerDir "d_sm_4");
	_vehicle setPos _poss;
	_vehicle lock true;
	_vehicle call d_fnc_addKilledEHSM;
	sleep 2.123;
	["specops", 1, "basic", 1, d_x_sm_pos select 1, 200, true] spawn d_fnc_CreateInf;
	sleep 2.123;
	["aa", 1, "tracked_apc", 1, "tank", 1, d_x_sm_pos select 2, 1, 400, true] spawn d_fnc_CreateArmor;
	d_x_sm_vec_rem_ar pushBack _vehicle;
};
