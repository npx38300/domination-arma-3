...
_colortouse = d_player_side call BIS_fnc_sideColor;
{
	...
	if (!isNil "_u" && {alive _u} && {!(_u getVariable ["xr_pluncon", false])} && {_u != player}) then {
...
if (_mode == 1) then {
	...
	_month = if (date select 1 < 10) then {format ["0%1", str (date select 1)]} else {str (date select 1)};;
	_day = if (date select 2 < 10) then {format ["0%1", str (date select 2)]} else {str (date select 2)};
	_hour = if (date select 3 < 10) then {format ["0%1", str (date select 3)]} else {str (date select 3)};
	_minute = if (date select 4 < 10) then {format ["0%1", str (date select 4)]} else {str (date select 4)};
	...
	1235 cutRsc ["d_sat_timode","PLAIN"];
	((uiNamespace getVariable "d_sat_timode") displayCtrl 50) ctrlSetText "Off";
	private ["_skipEH"];
	_skipEH = (findDisplay 46) displayAddEventHandler ["KeyDown", {
...
waitUntil {camPreloaded BIS_fnc_establishingShot_fakeUAV || {!isNil "BIS_fnc_establishingShot_skip"} || {player getVariable ["xr_pluncon", false]} || {!alive player}};
...
if (isNil "BIS_fnc_establishingShot_skip") then {
	...
	[_pos, _alt, _rad, _ang, _dir] spawn {
		...
		while {isNil "d_exit_fake_uav" && {!( player getVariable ["xr_pluncon", false])} && {alive player}} do {
			_ang = _ang - 0.5;
			...
			waitUntil {camCommitted BIS_fnc_establishingShot_fakeUAV || {!isNil "d_exit_fake_uav"} || {player getVariable ["xr_pluncon", false]} || {!alive player}};
	...
	if (isNil "BIS_fnc_establishingShot_skip" && {!( player getVariable ["xr_pluncon", false])} && {alive player}) then {
		enableEnvironment true;
		2 fadeSound 1;

		// Static fade-in
		("BIS_layerStatic" call BIS_fnc_rscLayer) cutRsc ["RscStatic", "PLAIN"];
		waitUntil {!isNull (uiNamespace getVariable "RscStatic_display") || {!isNil "BIS_fnc_establishingShot_skip"} || {player getVariable ["xr_pluncon", false]} || {!alive player}};
		waitUntil {isNull (uiNamespace getVariable "RscStatic_display")  || {!isNil "BIS_fnc_establishingShot_skip"} || {player getVariable ["xr_pluncon", false]} || {!alive player}};
			...
			if (_mode == 0) then {
				...
				if (isNil "BIS_fnc_establishingShot_skip" && {!(player getVariable ["xr_pluncon", false])} && {alive player}) then {
					...
					*/
					...
					waitUntil {scriptDone BIS_fnc_establishingShot_SITREP || {!isNil "BIS_fnc_establishingShot_skip"} || {player getVariable ["xr_pluncon", false]} || {!alive player}};
					
					private ["_time"];
					_time = time + 2;
					waitUntil {time >= _time || {isNil "BIS_fnc_establishingShot_skip"} || {player getVariable ["xr_pluncon", false]} || {!alive player}};
