...
private ["_state", "_vec", "_hud", "_control", "_chopttype", "_hudoff", "_nobjects", "_nearest", "_dummy", "_chdispx", "_nx", "_ny", "_px", "_py", "_control2", "_angle", "_a", "_b", "_dif", "_chdispx2", "_controlxx","_distvstr", "_helper_h", "_doexitit", "_chophud_shown", "_chophud2_shown"];

_helper_h = d_HeliHEmpty createVehicleLocal [0,0,0];
_helper_h enableSimulation false;

disableSerialization;

_ui_forward = "pics\ui_tankdir_forward_ca.paa";
_ui_back = "pics\ui_tankdir_back_ca.paa";
_ui_left = "pics\ui_tankdir_left_ca.paa";
_ui_right = "pics\ui_tankdir_right_ca.paa";
_ui_tohigh = "pics\ui_tankdir_turret_ca.paa";
_ui_ok = "pics\ui_tankdir_tower_ca.paa";
...
while {d_player_in_vec} do {
	if (player == driver _vec) then {
		_chopttype = _vec getVariable ["d_choppertype", -1];
		__TRACE_2("","_vec","_chopttype")
		if (_chopttype > -1) then {
			_chop_welcome_scr = [_chopttype,_vec] spawn d_fnc_chopper_welcome2;
			sleep 0.321;
		...
		if (_chopttype in [0,1]) then {
			...
			while {d_player_in_vec && {alive player} && {player == driver _vec}} do {
				if (d_chophud_on && {!visibleMap}) then {
					...
					if (_vec getVariable ["d_Vehicle_Attached", false]) then {
						// no need to check for nearest!
						_nearest = _vec getVariable ["d_Attached_Vec", objNull];
						_check_cond = !isNull _nearest;
					} else {
						...
						if (!isNull _nearest) then {
							private "_marp";
							_marp = _nearest getVariable ["d_WreckMaxRepair", d_WreckMaxRepair];
							...
							_check_cond = if (_chopttype == 1) then {
								...
								(!isNull _nearest && {(getPosATLVisual _vec) select 2 > 2.5} && {(_nearestu in _possible_types)})
							};
						};
					};
					
					sleep 0.001;
					__TRACE_3("","_vec","_check_cond","_nearest")
					
					if (_check_cond) then {
						if (!_chophud_shown) then {
							67321 cutRsc ["d_chopper_lift_hud", "PLAIN"];
							_chophud_shown = true;
						...
						if (_prev_nearest != _nearest) then {
							_tofn = typeOf _nearest;
							
							_type_name = [_tofn, "CfgVehicles"] call d_fnc_GetDisplayName;
							__CTRL2(64438) ctrlSetText (if !(_vec getVariable ["d_Vehicle_Attached", false]) then {format [_typestr, _type_name]} else {format [_liftstr, _type_name]});

							_picstat = getText (configFile/"CfgVehicles"/_tofn/"picture");
						...
						if (isPipEnabled && {(!_pip_cam_on || {_prev_nearest != _nearest})}) then {
							...
							_cam_loc = _vec selectionPosition "slingcamera";
							...
							_sling_cam camPreparetarget _nearest;
						...
						if !(_vec getVariable ["d_Vehicle_Attached", false]) then {
							...
							_nearest_pos = getPosATLVisual _nearest;
							_pos_vec = getPosATLVisual _vec;
							...
							_ddx1 = if (!surfaceIsWater (getPosASL _nearest)) then {
								round ((getPosATLVisual _nearest) select 2)
							} else {
								round ((getPosASLVisual _nearest) select 2)
							};
						...
						if (_chophud_shown) then {
							67321 cutRsc ["d_Empty", "PLAIN"];
							_chophud_shown = false;
						};
						if (!_chophud2_shown) then {
							67322 cutRsc ["d_chopper_lift_hud2", "PLAIN"];
