...
{_p linkItem _x} forEach (d_custom_layout select 0);
_bino = d_custom_layout select 17;
if (_bino != "") then {
	_p addWeapon _bino;
};

_runi = d_custom_layout select 9;
__TRACE_1("","_runi")
removeUniform _p;
_p addUniform _runi;

_rvest = d_custom_layout select 10;
__TRACE_1("","_rvest")
removeVest _p;
_p addVest _rvest;

_backp = d_custom_layout select 11;
__TRACE_1("","_backp")
...
_backbackp = d_custom_layout select 15;
...
_magsammofull =+ (d_custom_layout select 1);
...
_rsecw = d_custom_layout select 4;
__TRACE_1("","_rsecw")
if (_rsecw != "") then {
	_p addWeapon _rsecw;		
	{if (_x != "") then {_p addSecondaryWeaponItem _x}} forEach (d_custom_layout select 3);
};
__TRACE_1("after sec weap","_magsammofull")

_rhandgw = d_custom_layout select 6;
if (_rhandgw != "") then {
	_p addWeapon _rhandgw;
	removeAllHandgunItems _p;
	{if (_x != "") then {_p addHandgunItem _x}} forEach (d_custom_layout select 7);
};
__TRACE_1("after handgun weap","_magsammofull")

_rprimw = d_custom_layout select 2;
if (_rprimw != "") then {
	_p addWeapon _rprimw;
	removeAllPrimaryWeaponItems _p;
	{if (_x != "") then {_p addPrimaryWeaponItem _x}} forEach (d_custom_layout select 3);
};
__TRACE_1("after prim weap","_magsammofull")

if (_bpadded) then {
	removeBackpackGlobal _p;
};

_gogs = d_custom_layout select 12;
__TRACE_1("","_gogs")
...
_headg = d_custom_layout select 13;
__TRACE_1("","_headg")
...

{
	__TRACE_1(" item","_x")
	_p addItem _x;
} forEach (d_custom_layout select 14);
