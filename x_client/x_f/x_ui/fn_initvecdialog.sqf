...
private ["_vec", "_caller", "_hasbox", "_ttyp", "_canloadunloadbox", "_pic", "_index", "_control", "_pos"];

_vec = d_curvec_dialog;
_caller = d_curcaller_dialog;

_display = (uiNamespace getVariable "d_VecDialog");
...
_vec_name = _vec getVariable "d_vec_name";
...
_hasbox = _vec getVariable ["d_ammobox", false];
_ttyp = toUpper (typeOf _vec);

_canloadunloadbox = _ttyp in d_check_ammo_load_vecs;
	...
	if ((_vec getVariable ["d_vec_type", ""]) == "MHQ") then {
		if !(_caller in _vec) then {
			lbClear 44449;
			{
				...
				_index = __control(44449) lbAdd ([_x, "CfgVehicles"] call d_fnc_GetDisplayName);
...
if (_move_controls) then {
	...
	_control = _display displayCtrl 44454;
...
0 spawn {
	waitUntil {!d_vec_dialog_open || {!alive player} || {player getVariable ["xr_pluncon", false]}};
