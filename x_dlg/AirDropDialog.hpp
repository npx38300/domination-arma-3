class D_AirDropDialog {
	idd = -1;
	movingEnable = true;
	onLoad = "uiNamespace setVariable ['D_AirDropDialog', _this select 0];[_this select 0] call bis_fnc_guiEffectTiles;call d_fnc_initairdropdialog;d_airdrop_dialog_open = true;d_do_ma_update_n = true";
	onUnLoad = "uiNamespace setVariable ['D_AirDropDialog', nil];d_airdrop_dialog_open = false;d_do_ma_update_n = false";
	...
	class controls {
		__CANCELCLOSEB($STR_DOM_MISSIONSTRING_1229,-1)
		class Drop3: RscButtonBase2 {
			idc = 11004;
		...
		class Map: D_RscMapControl {
			...
			onMouseButtonClick = "if (ctrlShown ((uiNamespace getVariable 'D_AirDropDialog') displayCtrl 11111)) then {_pp = (_this select 0) ctrlMapScreenToWorld [_this select 2, _this select 3];'d_drop_zone' setMarkerPos _pp}";
		};
	};
};
