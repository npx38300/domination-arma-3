class xr_SpectDlg {
	idd = -1;
	movingEnable = 1;
	onLoad = "uiNamespace setVariable ['XR_SpectDlg', _this select 0];d_do_ma_update_n = true;[_this select 0, 1] call d_fnc_teleportdialoginit";
	onUnload = "uiNamespace setVariable ['XR_SpectDlg', nil];d_do_ma_update_n = false;d_x_loop_end = true";
	class controls {
		...
		class MapButton: RscButtonBase2 {
			idc = 1002;
			text = "$STR_DOM_MISSIONSTRING_1333"; 
			action = "_ctrl = (uiNamespace getVariable 'XR_SpectDlg') displayCtrl 1000; if (ctrlShown _ctrl) then {ctrlShow [1000, false];((uiNamespace getVariable 'XR_SpectDlg') displayCtrl 1003) ctrlSetText (localize 'STR_DOM_MISSIONSTRING_1334');};_ctrl = (uiNamespace getVariable 'XR_SpectDlg') displayCtrl 3000;if (ctrlShown _ctrl) then {ctrlShow [3000, false];((uiNamespace getVariable 'XR_SpectDlg') displayCtrl 1002) ctrlSetText (localize 'STR_DOM_MISSIONSTRING_1333');d_x_loop_end = true;} else {ctrlShow [3000, true];((uiNamespace getVariable 'XR_SpectDlg') displayCtrl 1002) ctrlSetText (localize 'STR_DOM_MISSIONSTRING_1333a'); call xr_fnc_updaterlb}";
		...
		class ShowPlayersButton: MapButton {
			...
			action = "_ctrl = (uiNamespace getVariable 'XR_SpectDlg') displayCtrl 3000; if (ctrlShown _ctrl) then {ctrlShow [3000, false];((uiNamespace getVariable 'XR_SpectDlg') displayCtrl 1002) ctrlSetText (localize 'STR_DOM_MISSIONSTRING_1333');};_ctrl = (uiNamespace getVariable 'XR_SpectDlg') displayCtrl 1000;if (ctrlShown _ctrl) then {ctrlShow [1000, false];((uiNamespace getVariable 'XR_SpectDlg') displayCtrl 1003) ctrlSetText (localize 'STR_DOM_MISSIONSTRING_1334')} else {ctrlShow [1000, true];((uiNamespace getVariable 'XR_SpectDlg') displayCtrl 1003) ctrlSetText (localize 'STR_DOM_MISSIONSTRING_1334a')}";
			x = "SafeZoneX + 0.03";
		};
